...
import { Typography, Container, Grid, Box, Chip, IconButton, Menu, MenuItem, ListItemIcon, ListItemText, Paper } from '@mui/material';
import FilterListIcon from '@mui/icons-material/FilterList';
import CategoryIcon from '@mui/icons-material/Category';

const categories = ['Salad', 'Juice', 'Lunch', 'Pizza', 'Burger', 'Fish', 'Soup', 'Breakfast', 'Local Breakfast'];
const items = [
        ...
        {items
          ...
          .map((item, index) => (
            <Grid item xs={12} sm={6} md={4} key={index}>
              <Paper elevation={3} variant="outlined" sx={{ p: 2, borderRadius: 3 }}>
                <Typography variant="h6" gutterBottom>{item.name}</Typography>
                <Typography variant="subtitle1" color="textSecondary" gutterBottom>{item.category}</Typography>
